    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="glass-panel rounded-[2rem] p-8 shadow-float flex flex-col justify-center bg-gradient-violet text-white">
          <p className="text-white/70 font-medium mb-1">Total Fuel Costs</p>
          <h2 className="text-4xl font-bold tracking-tight"><MonoNumber value={data.totalFuelCosts || 0} prefix="$" /></h2>
        </div>
      </div>
      <div className="glass-panel rounded-[1.75rem] p-6">
        <h3 className="font-semibold text-foreground/80 mb-4 flex items-center gap-2"><Clock size={18}/> Pending Expenses Awaiting Approval</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border/50">
              {data.pendingExpenses && data.pendingExpenses.length > 0 ? data.pendingExpenses.map(e => (
                <tr key={e.id}>
